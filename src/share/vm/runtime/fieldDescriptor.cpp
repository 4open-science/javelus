...
void fieldDescriptor::reinitialize(InstanceKlass* ik, int index) {
  if (_cp.is_null() || (field_holder() != ik)) {
    ...
    assert(field_holder() == ik || ik->is_inplace_new_class(), "must be already initialized to this class");
  }
...
void fieldDescriptor::print_on_for(outputStream* st, oop obj) {
  assert(!(obj->mark()->is_mixed_object() && dsu_flags().needs_mixed_object_check()), "caller should do mixed object check");
  print_on(st);
